...
// GetTranscript retrieves a transcript for an audiobook
func (d *DatabaseService) GetTranscript(audiobookID uuid.UUID) (*models.Transcript, error) {
	query := `
		SELECT content
		FROM transcripts
	...
	err := d.db.QueryRow(query, audiobookID).Scan(&transcript.Content)
	if err != nil {
...
func (d *DatabaseService) SaveAIOutput(output *models.AIOutput) error {
	query := `
		INSERT INTO ai_outputs (id, audiobook_id, output_type, content, model_used, created_at)
		VALUES ($1, $2, $3, $4, $5, $6)
		ON CONFLICT (audiobook_id, output_type) 
		DO UPDATE SET 
	...
	_, err := d.db.Exec(query,
		output.ID, output.AudiobookID, output.OutputType,
		output.Content, output.ModelUsed, output.CreatedAt,
	)
	if err != nil {
		return fmt.Errorf("failed to save AI output: %v", err)
	}

	log.Printf("Saved AI output for audiobook %s, type: %s", output.AudiobookID, output.OutputType)
	return nil
}
