...
// RedisConsumer handles consuming jobs from Redis queues
...
// JobMessage represents a job message from the queue
...
// ConsumeJobs starts consuming jobs from the specified queue
func (r *RedisConsumer) ConsumeJobs(ctx context.Context, jobType string, processor func(JobMessage) error) error {
	...
	for {
		...
		default:
			...
			log.Printf("Processing job %s (type: %s) for audiobook %s", message.ID, message.JobType, message.AudiobookID)
			...
			if err := processor(message); err != nil {
				log.Printf("Error processing job %s: %v", message.ID, err)
...
func (r *RedisConsumer) GetQueueStats(ctx context.Context) (map[string]interface{}, error) {
	jobTypes := []string{"summarize", "tag", "embed"}
